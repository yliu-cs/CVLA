...
def get_args():
    ...
    args.figure_dir = os.path.join(args.figure_dir, "dataset", "comment")
    return args


def get_comment_number(info_path):
    return json.loads(open(info_path, "r", encoding="utf8").read())["comment_number"]


def plot_comment(args):
    ...
    for vdir in tqdm(os.listdir(unlabeled_video_dir), desc=f"Like from {os.path.normpath(unlabeled_video_dir).split(os.sep)[-1]}", ncols=100):
        like = get_comment_number(os.path.join(unlabeled_video_dir, vdir, "info.json"))
        unlabeled_comments.append(like)
    unlabeled_comments_cnt = Counter(list(map(len, map(str, unlabeled_comments))))
    unlabeled_comments = [unlabeled_comments_cnt[i] for i in range(8)]
    unlabeled_comments = [sum(unlabeled_comments[:4])] + unlabeled_comments[4:-2] + [sum(unlabeled_comments[-2:])]
    
    labeled_video_dir = os.path.join(args.dataset_dir, "labeled")
    labeled_comments = {"pos": [], "neg": []}
    for vdir in tqdm(os.listdir(labeled_video_dir), desc=f"Like from {os.path.normpath(labeled_video_dir).split(os.sep)[-1]}", ncols=100):
        like = get_comment_number(os.path.join(labeled_video_dir, vdir, "info.json"))
        if json.loads(open(os.path.join(labeled_video_dir, vdir, "info.json"), "r", encoding="utf8").read())["humor"]:
            labeled_comments["pos"].append(like)
        else:
            labeled_comments["neg"].append(like)
    labeled_comments_cnt = {}
    for lbl in labeled_comments.keys():
        labeled_comments_cnt = Counter(list(map(len, map(str, labeled_comments[lbl]))))
        labeled_comments[lbl] = [labeled_comments_cnt[i] for i in range(8)]
        labeled_comments[lbl] = [sum(labeled_comments[lbl][:4])] + labeled_comments[lbl][4:-2] + [sum(labeled_comments[lbl][-2:])]
    ...
    for theme_idx, theme in enumerate(list(filter(lambda x: len(x) == 3, Theme))):
        ...
        ax1.xaxis.set_major_formatter(ticker.FuncFormatter(lambda x, pos: "100" if x == 0 else (f"{int(10 ** (x - 1))}K" if 1 <= x <= 3 else f"{int(10 ** (x - 4))}M")))
        # ax1.set_xticks(list(range(5)), labels=["0.5k"] + [f"{10 ** (i + 4)}{}" for i in range(1, 5)])
        y_max = max(math.ceil(np.max(unlabeled_comments) / 1000), math.ceil(np.max(np.array(labeled_comments['pos']) + np.array(labeled_comments['neg'])) / 100)) * 1000
        ...
        ax2.bar(
            list(map(lambda x: x + 0.5 + width / 2, list(range(4))))
            , labeled_comments["neg"]
            , bottom=labeled_comments["pos"]
            , width=width
            , color="#F9F8F4"
            , edgecolor=f"#{theme[2]}"
            , hatch="\\\\\\"
        ...
        y_max = int(y_max // 10)
        # y_max = (math.ceil(np.max(np.array(labeled_comments['pos']) + np.array(labeled_comments['neg'])) / 100) + 1) * 100
